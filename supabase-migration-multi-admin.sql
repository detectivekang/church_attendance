-- =========================================================
-- 마이그레이션: 공동 운영자(admin) 지정 기능 추가
-- =========================================================
-- 이미 supabase-schema.sql을 한 번 실행하신 상태라면, 전체를 다시 돌리지
-- 말고 이 파일만 SQL Editor에서 실행하세요. (함수 재정의라 안전하게 다시
-- 실행 가능합니다)
...
begin
  ...
  if v_collection = 'roles' then
    ...
    -- UPDATE: 형태 정합성 + 컨텍스트 1개 추가/제거(또는 none→역할 교체)만 허용
    if not (

...
async function checkNoticePopups() {
  ...
  try {
    ...
    snap = await churchCol("notices").where("popup", "==", true).get();
  } catch (e) {
...
async function loadNotices() {
  const snap = await churchCol("notices")
...
async function turnOffAllPopups() {
  const snap = await churchCol("notices").where("popup", "==", true).get();
...
function renderNoticeAdminList() {
  ...
  list.querySelectorAll("[data-del]").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      await churchCol("notices").doc(btn.dataset.del).delete();
      await loadNotices();
      renderNoticeAdminList();
    });
  });
